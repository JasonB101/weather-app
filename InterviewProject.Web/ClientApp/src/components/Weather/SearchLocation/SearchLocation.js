...
const SearchLocation = (props) => {
    ...
    const onChangeSearchField = (e) => {
        if (searchField.length === 3) {
            populatePossibleLocations(searchField);
        }
        changeSearchField(e.target.value);
        
    }

    const locations = possibleLocations.map(location => <Location key={location.id} props={location}/>)
    ...
    // setPossibleLocations(data.consolidated_weather);
    setPossibleLocations(data.consolidated_weather);
    } 
